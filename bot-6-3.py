...
COR_CLEAN = 0x367BF0   # azul kali — info/ok
COR_RED   = 0xE06C75   # vermelho kali — perigo/ban
COR_DIM   = 0x56B6C2   # ciano kali — neutro/log
COR_WARN  = 0xF0A500   # laranja — aviso/warning

def kprompt(cmd: str = "", guild_name: str = "server") -> str:
    """Prompt estilo Kali Linux."""
    return f"┌──(aegis㉿{guild_name})-[~]\n└─$ {cmd}"
...
async def checar_vpn(member: discord.Member) -> bool:
    ...
    try:
        ...
        idade = (datetime.now(timezone.utc).replace(tzinfo=None) - member.created_at.replace(tzinfo=None)).days
...
async def log_externo(evento: str, cor: int = None, detalhes: str = ""):
    ...
    payload = {
        "embeds": [{
            ...
            "footer": {"text": f"[aegis] {datetime.now(timezone.utc).replace(tzinfo=None).strftime('%Y-%m-%d %H:%M:%S UTC')}"}
        }]
    }
    try:
        async with aiohttp.ClientSession() as session:
...
def ts(): return int(datetime.now(timezone.utc).replace(tzinfo=None).timestamp())
...
def registrar(tipo: str, detalhe: str):
    config["historico"].append({"tipo": tipo, "detalhe": detalhe, "time": datetime.now(timezone.utc).replace(tzinfo=None).isoformat()})
...
def embed_auto(acao, motivo, usuario, nivel=None):
    ...
    ts = datetime.now(timezone.utc).replace(tzinfo=None).strftime('%H:%M:%S UTC')
    e.set_footer(text=f"└─$ aegis  •  {ts}")
    return e
...
def embed_status():
    ...
    e.set_thumbnail(url="https://www.kali.org/images/kali-dragon-icon.svg")
    e.description = (
        "```ansi\n"
    ...
    e.set_footer(text=f"└─$ aegis  •  {datetime.now(timezone.utc).replace(tzinfo=None).strftime('%H:%M:%S UTC')}")
    return e
...
def embed_ajuda():
    ...
    e.add_field(name="moderacao", value=(
        ...
        "  /reset  @u\n"
        "  /rep    @u\n"
        "  /apelacao lista/aceitar/rejeitar\n"
        "```"
    ), inline=False)
    ...
    e.set_footer(text="└─$ help  |  duracao: 10s 5m 2h 1d")
    return e
...
async def punir(member: discord.Member, motivo: str, guild: discord.Guild):
    ...
    try:
        ...
        dm.set_footer(text="└─$ exit // session closed")
        await member.send(embed=dm)
    except Exception: pass
...
async def registrar_apelacao(user: discord.User, motivo: str):
    apelacoes[user.id] = {
        "user"  : str(user),
        "motivo": motivo,
        "status": "pendente",
...
async def ativar_lockdown(guild: discord.Guild, auto=False, modo: str = None):
    ...
    raid_log.append({"time": datetime.now(timezone.utc).replace(tzinfo=None).isoformat(), "auto": auto, "modo": modo})
...
async def ativar_emergencia(guild: discord.Guild, motivo: str):
    ...
    config["lockdown"]   = True
    config["total_raids"] += 1
    raid_log.append({"time": datetime.now(timezone.utc).replace(tzinfo=None).isoformat(), "auto": True})
...
async def enviar_captcha(member: discord.Member, guild: discord.Guild):
    ...
    captcha_pendente[member.id] = {
        "codigo": codigo,
        "guild" : guild.id,
        "expira": datetime.now(timezone.utc).replace(tzinfo=None) + timedelta(minutes=5),
    }
    e = discord.Embed(color=COR_CLEAN)
    e.set_author(name=f"verificacao — {guild.name}")
    e.description = f"para acessar o servidor, responda com o codigo:\n\n```\n  {codigo}\n```\n> voce tem 5 minutos."
    e.set_footer(text="└─$ captcha --verify")
    try:
...
async def fazer_backup(guild: discord.Guild):
    ...
    backup_data["timestamp"] = datetime.now(timezone.utc).replace(tzinfo=None).isoformat()
...
async def reset_daily():
    ...
    hoje = datetime.now(timezone.utc).replace(tzinfo=None).date().isoformat()
...
async def resumo_semanal():
    ...
    for guild in client.guilds:
        hist = [h for h in config["historico"]
                if datetime.now(timezone.utc).replace(tzinfo=None) - datetime.fromisoformat(h["time"]) <= timedelta(days=7)]
...
async def on_message(message: discord.Message):
    ...
    if isinstance(message.channel, discord.DMChannel):
        ...
        if uid in captcha_pendente:
            ...
            if datetime.now(timezone.utc).replace(tzinfo=None) > dados["expira"]:
        ...
        # Sistema de apelação via DM
            ...
            if motivo:
                await registrar_apelacao(message.author, motivo)
    ...
    uid, guild, now = message.author.id, message.guild, datetime.now(timezone.utc).replace(tzinfo=None)
...
@tree.command(name="apelacao", description="gerencia apelacoes de usuarios banidos")
@app_commands.checks.has_permissions(administrator=True)
async def cmd_apelacao(interaction: discord.Interaction, acao: str, user_id: str = None):
    if acao == "lista":
        if not apelacoes:
...
KALI_LOGO = "https://www.kali.org/images/kali-dragon-icon.svg"
...
class PainelView(discord.ui.View):
    ...
    def embed_pagina(self):
        ...
        if self.pagina == "security":
            import asyncio
            return self._embed_security_sync()
        return embed_status()

    def _embed_security_sync(self):
        agora = datetime.now(timezone.utc).replace(tzinfo=None)
        while joins_por_minuto and agora - joins_por_minuto[0] > timedelta(seconds=60):
            joins_por_minuto.popleft()
        tl  = config["threat_level"]
        cor = {"normal": COR_CLEAN, "warning": COR_WARN, "critical": COR_RED}.get(tl, COR_CLEAN)
        e   = discord.Embed(color=cor)
        e.set_author(name="┌──(aegis㉿server)-[~]  └─$ security --live")
        e.set_thumbnail(url=KALI_LOGO)
        tl_icon = {"normal": "[ OK ]", "warning": "[WARN]", "critical": "[CRIT]"}.get(tl, "[ ?? ]")
        e.description = (
            f"```ansi\n"
            f"  threat      {tl_icon} {tl.upper()}\n"
            f"  joins/min   {len(joins_por_minuto)}\n"
            f"  joins hoje  {config['daily_joins']}\n"
            f"  raids hoje  {config['daily_raids']}\n"
            f"  warns hoje  {config['daily_warns']}\n"
            f"  lockdown    {'ON  -- ' + config['lockdown_mode'] if config['lockdown'] else 'OFF'}\n"
            f"  emergencia  {'ON' if config['emergencia'] else 'OFF'}\n"
            f"  shadow muted {len(config['shadow_muted'])}\n"
            f"  ia risk     {config['raid_score']}/10\n"
            f"```"
        )
        e.set_footer(text=f"└─$ aegis  •  {agora.strftime('%H:%M:%S UTC')}")
        return e

    def _embed_protecoes(self):
        def s(v): return "[ ON ]" if v else "[ -- ]"
        e = discord.Embed(color=COR_CLEAN)
        e.set_author(name="┌──(aegis㉿server)-[~]  └─$ protecoes --list")
        e.set_thumbnail(url=KALI_LOGO)
        e.description = (
            "```ansi\n"
            f"  anti-raid      {s(config['antiraid'])}\n"
            f"  anti-spam      {s(config['antispam'])}\n"
            f"  anti-phishing  {s(config['antiphishing'])}\n"
            f"  anti-zalgo     {s(config['anti_zalgo'])}\n"
            f"  anti-invite    {s(config['anti_invite'])}\n"
            f"  anti-repeated  {s(config['anti_repeated'])}\n"
            f"  anti-nuke      {s(config['anti_nuke'])}\n"
            ...
            f"  shadow mute    {s(config['shadow_mute'])}\n"
            "```\n"
            "> use os botoes abaixo para alternar"
        )
        e.set_footer(text="└─$ aegis  •  clique pra togglear")
        return e

    def _embed_shadow(self):
        e = discord.Embed(color=COR_DIM)
        e.set_author(name="┌──(aegis㉿server)-[~]  └─$ shadow --list")
        e.set_thumbnail(url=KALI_LOGO)
        if not config["shadow_muted"]:
            e.description = "```\n  [empty]  nenhum shadow mute ativo.\n```"
        else:
            linhas = "\n".join(f"  {uid}" for uid in list(config["shadow_muted"])[:20])
            e.description = f"```ansi\n  {len(config['shadow_muted'])} usuario(s) em shadow mute:\n\n{linhas}\n```"
        e.set_footer(text="└─$ /sunmute @user")
        return e

    def _embed_apelacoes(self):
        e = discord.Embed(color=COR_WARN)
        e.set_author(name="┌──(aegis㉿server)-[~]  └─$ apelacoes --pending")
        e.set_thumbnail(url=KALI_LOGO)
        pendentes = {uid: d for uid, d in apelacoes.items() if d["status"] == "pendente"}
        if not pendentes:
            e.description = "```\n  [empty]  nenhuma apelacao pendente.\n```"
        else:
            linhas = ""
            for uid, d in list(pendentes.items())[:10]:
                linhas += f"  {uid}  {d['user']}\n  > {d['motivo'][:40]}\n\n"
            e.description = f"```ansi\n  {len(pendentes)} pendente(s):\n\n{linhas}```"
        e.set_footer(text="└─$ /apelacao aceitar|rejeitar <id>")
        return e

    def _embed_reputacao(self):
        e = discord.Embed(color=COR_CLEAN)
        e.set_author(name="┌──(aegis㉿server)-[~]  └─$ rep --top")
        e.set_thumbnail(url=KALI_LOGO)
        if not reputacao:
            e.description = "```\n  [empty]  sem dados de reputacao.\n```"
        else:
            ...
            linhas = "  -- piores --\n"
            for uid, r in piores:
                linhas += f"  {uid:<20} {'+' if r >= 0 else ''}{r}  ({reputacao_str(uid)})\n"
            linhas += "\n  -- melhores --\n"
            for uid, r in melhores:
                linhas += f"  {uid:<20} {'+' if r >= 0 else ''}{r}  ({reputacao_str(uid)})\n"
            e.description = f"```ansi\n{linhas}```"
        e.set_footer(text="└─$ /rep @user +N")
        return e
    ...
    async def btn_status(self, interaction, button):
        self.pagina = "status"; self._nav(); await interaction.response.edit_message(embed=self.embed_pagina(), view=self)

    @discord.ui.button(label="protecoes",   style=discord.ButtonStyle.secondary, row=0)
    async def btn_protecoes(self, interaction, button):
        self.pagina = "protecoes"; self._nav(); await interaction.response.edit_message(embed=self.embed_pagina(), view=self)

    @discord.ui.button(label="security",    style=discord.ButtonStyle.secondary, row=0)
    async def btn_security(self, interaction, button):
        self.pagina = "security"; self._nav(); await interaction.response.edit_message(embed=self.embed_pagina(), view=self)

    @discord.ui.button(label="shadow",      style=discord.ButtonStyle.secondary, row=0)
    async def btn_shadow(self, interaction, button):
        self.pagina = "shadow"; self._nav(); await interaction.response.edit_message(embed=self.embed_pagina(), view=self)

    @discord.ui.button(label="apelacoes",   style=discord.ButtonStyle.secondary, row=0)
    async def btn_apelacoes(self, interaction, button):
        self.pagina = "apelacoes"; self._nav(); await interaction.response.edit_message(embed=self.embed_pagina(), view=self)

    @discord.ui.button(label="reputacao",   style=discord.ButtonStyle.secondary, row=1)
    async def btn_rep(self, interaction, button):
        self.pagina = "rep"; self._nav(); await interaction.response.edit_message(embed=self.embed_pagina(), view=self)

    # ── ACOES RAPIDAS ────────────────────────────────────────────

    @discord.ui.button(label="lock/unlock", style=discord.ButtonStyle.danger,    row=2)
    async def btn_lock(self, interaction, button):
        if config["lockdown"]: await desativar_lockdown(interaction.guild)
        else:                  await ativar_lockdown(interaction.guild)
        await interaction.response.edit_message(embed=self.embed_pagina(), view=self)

    @discord.ui.button(label="emergencia",  style=discord.ButtonStyle.danger,    row=2)
    async def btn_emerg(self, interaction, button):
        if config["emergencia"]: await desativar_emergencia(interaction.guild)
        else:                    await ativar_emergencia(interaction.guild, "painel")
        await interaction.response.edit_message(embed=self.embed_pagina(), view=self)
    ...
    @discord.ui.button(label="toggle ia",   style=discord.ButtonStyle.secondary, row=3)
    async def btn_ia(self, interaction, button):
        config["ai_mode"] = not config["ai_mode"]
        await interaction.response.edit_message(embed=self.embed_pagina(), view=self)

    @discord.ui.button(label="silencioso",  style=discord.ButtonStyle.secondary, row=3)
    async def btn_silencioso(self, interaction, button):
        config["silencioso"] = not config["silencioso"]
        await interaction.response.edit_message(embed=self.embed_pagina(), view=self)

    @discord.ui.button(label="captcha",     style=discord.ButtonStyle.secondary, row=3)
    async def btn_captcha(self, interaction, button):
        config["captcha"] = not config["captcha"]
        await interaction.response.edit_message(embed=self.embed_pagina(), view=self)

    @discord.ui.button(label="salvar db",   style=discord.ButtonStyle.success,   row=3)
    ...
    def _nav(self):
        paginas = {
            "status": self.btn_status, "protecoes": self.btn_protecoes,
            "security": self.btn_security, "shadow": self.btn_shadow,
            "apelacoes": self.btn_apelacoes, "rep": self.btn_rep,
        }
        for nome, btn in paginas.items():
...
async def cmd_ver(interaction: discord.Interaction, usuario: discord.Member):
    ...
    idade = (datetime.now(timezone.utc).replace(tzinfo=None) - usuario.created_at.replace(tzinfo=None)).days
