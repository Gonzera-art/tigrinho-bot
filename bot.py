...
def embed_ajuda():
    ...
    e.add_field(name="📖 tutorial do bot Aegis - by gon", value=(
        "```\n"
        "  1. PRIMEIRO USO\n"
        "     /setup → cria cargos e canais automaticamente\n"
        "     defina LOG_CHANNEL_ID e STAFF_ROLE_ID no topo do arquivo\n"
        "\n"
        "  2. RAID ACONTECENDO AGORA\n"
        "     /lock hard  → trava tudo pra todo mundo\n"
        "     /emergencia → expulsa entrantes + apaga convites\n"
        "     /security   → dashboard com joins/min e threat level\n"
        "     /unlock     → reabre quando acabar\n"
        "\n"
        "  3. LOCKDOWN MODES\n"
        "     soft   → so slowmode (conversa continua)\n"
        "     medium → bloqueia envio do @everyone\n"
        "     hard   → bloqueia tudo, so staff le e escreve\n"
        "\n"
        "  4. HONEYPOT\n"
        "     /honeypot #canal → canal invisivel pra membros normais\n"
        "     qualquer membro que mandar msg = ban automatico\n"
        "     ideal pra pegar raiders e bots na entrada\n"
        "\n"
        "  5. VPN / PROXY\n"
        "     coloque VPN_API_KEY no topo do arquivo (vpnapi.io)\n"
        "     membro entrou via VPN = quarentena automatica\n"
        "\n"
        "  6. ML SPAM SCORE\n"
        "     score 0-10 em cada mensagem via patterns ponderados\n"
        "     score 5-7 = aviso | score 8+ = delete + punicao\n"
        "     /spamscore <texto> pra testar qualquer mensagem\n"
        "\n"
        "  7. SIMILARITY SPAM\n"
        "     detecta spam disfarçado (c0mpr4 = compra)\n"
        "     3+ msgs com 85% similaridade = punido\n"
        "\n"
        "  8. RAID FINGERPRINT\n"
        "     bot aprende o padrao dos raids anteriores\n"
        "     eleva threat pra WARNING antes de atingir limite\n"
        "\n"
        "  9. AUTO NUKE RECOVERY\n"
        "     canais deletados em massa = restaura do backup sozinho\n"
        "     sem precisar de /backup manual\n"
        "\n"
        " 10. LOG EXTERNO\n"
        "     coloque WEBHOOK_EXTERNO no topo do arquivo\n"
        "     raids, nukes e honeypot vao pra um servidor separado\n"
        "     servidor nukado? voce ainda ve o log em outro lugar\n"
        "\n"
        " 11. PALAVRAS BANIDAS\n"
        "     /bw palavra leve  → avisa 1a vez, pune se repetir\n"
        "     /bw palavra grave → bane sem aviso (slur/odio)\n"
        "\n"
        " 12. DADOS SALVOS\n"
        "     tudo vai pro aegis.db automaticamente a cada 2min\n"
