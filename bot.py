...
load_dotenv()  # lê as variáveis do arquivo .env (não suba esse arquivo pro GitHub!)

TOKEN = os.getenv("DISCORD_TOKEN", "TOKEN_DO_DISCORD")
PREFIX = "!"
DONO_ID = 1277801876523454547
CANAL_BOT = 1513290347973967962

# Banner usado no painel !ajuda (precisa estar na pasta "assets" junto do script do bot)
BANNER_AJUDA_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets", "banner_ajuda.jpg")
BANNER_AJUDA_NOME = "banner_ajuda.jpg"
...
# Chance de vitória da aposta base (!apostar). Reduzida drasticamente para tornar
# a aposta mais arriscada e dar valor real aos itens da loja que melhoram as chances.
CHANCE_APOSTAR = 0.15  # 15% de chance de ganhar
...
ITENS = {
    ...
    "dado_viciado": {"nome": "🎲 Dado Viciado", "preco": 700, "desc": "Sua próxima aposta tem 50% de chance de vitória (em vez dos 15% normais)"},
    "chave_cofre": {"nome": "🔑 Chave do Cofre", "preco": 1200, "desc": "Garante a vitória da sua próxima aposta"},
    "kit_pesca": {"nome": "🎣 Kit de Pesca Turbo", "preco": 350, "desc": "Dobra o ganho da próxima pescaria"},
    "luva_caca": {"nome": "🧤 Luva do Caçador", "preco": 350, "desc": "Dobra o ganho da próxima caçada"},
    "broca_diamante": {"nome": "⛏️ Broca de Diamante", "preco": 350, "desc": "Dobra o ganho da próxima mineração"},
    "pingente_liberdade": {"nome": "🕊️ Pingente da Liberdade", "preco": 600, "desc": "Remove instantaneamente o seu status de Procurado"},
    "carta_imunidade": {"nome": "🛂 Carta de Imunidade", "preco": 900, "desc": "Você não paga multa na próxima tentativa de roubo, mesmo se for pego"},
}
...
class QuizView(View):
    def __init__(self, pergunta, fichas_ganho, fichas_perda, channel_id, autor_id):
    ...
    async def responder(self, interaction, escolha):
        ...
        if interaction.user.id != self.autor_id:
            await interaction.response.send_message("❌ Esse quiz é individual! Só quem usou `!quiz` pode responder. Inicie o seu com `!quiz`.", ephemeral=True)
            return
        ...
        if escolha == correta:
            ...
            registrar_evento(dados, "quiz", f"🧠 {interaction.user.display_name} acertou o quiz e ganhou {self.fichas_ganho} <:fichas:1517923672575185048>.")
            salvar(dados)
            await interaction.response.edit_message(
                content=f"🏆 **{interaction.user.display_name}** acertou! A resposta era **{correta}**!\n💰 Ganhou **{self.fichas_ganho} <:fichas:1517923672575185048>!**",
                view=None
            )
            for m in mensagens_extra:
            ...
            salvar(dados)
            await interaction.response.edit_message(
                content=f"❌ **{interaction.user.display_name}** errou! A resposta era **{correta}**. Perdeu **{self.fichas_perda} <:fichas:1517923672575185048>**.",
                view=None
            )
            for m in mensagens_extra:
                await interaction.followup.send(m)
    ...
    async def on_timeout(self):
        if not self.finalizado:
            ...
            if self.channel_id in quizzes_ativos:
                ...
                try:
                    await msg.edit(
                        content=f"⏰ **Tempo esgotado!** Você não respondeu a tempo. A resposta era **{self.pergunta['r']}**!",
                        view=None
                    )
                except:
                    pass
...
COMANDOS_MOD = {
    ...
    "darfichas", "tirfichas", "setfichas", "godmode", "cheattudo",
    "zerarfichas", "resetlimite", "resetquiz", "dano2x",
    "cancelluta", "premiarsemana", "sortearloteria",
    "addparceria", "removerparceria", "invocar", "painelparcerias"
}
...
async def pescar(ctx):
    ...
    boost_msg = ""
    if u.get("pesca_boost"):
        ganho *= 2
        u["pesca_boost"] = False
        boost_msg = " 🎣 **(Kit de Pesca Turbo dobrou o ganho!)**"
    ...
    salvar(dados)
    await ctx.send(f"{msg_pesca}\n💰 **+{ganho} <:fichas:1517923672575185048>**{mult_str}{boost_msg} (**{restantes}/3** restantes hoje)")
    if ev_aleatorio:
        texto_ev, delta_ev = ev_aleatorio
...
async def cacar(ctx):
    ...
    if u.get("caca_boost"):
        ganho *= 2
        u["caca_boost"] = False
        boost_msg = " 🧤 **(Luva do Caçador dobrou o ganho!)**"
    u["fichas"] += ganho
    u["total"] += ganho
...
async def minerar(ctx):
    ...
    boost_msg = ""
    if u.get("minerar_boost"):
        ganho *= 2
        u["minerar_boost"] = False
        boost_msg = " ⛏️ **(Broca de Diamante dobrou o ganho!)**"
    u["fichas"] += ganho
    u["total"] += ganho
...
async def apostar(ctx, valor: int):
    ...
    # Define a chance de vitória desta aposta (pode ser alterada por itens usados antes)
    if u.get("aposta_garantida"):
        chance_vitoria = 1.0
        u["aposta_garantida"] = False
        usou_chave = True
    elif u.get("dado_viciado_ativo"):
        chance_vitoria = 0.5
        u["dado_viciado_ativo"] = False
        usou_chave = False
    else:
        chance_vitoria = CHANCE_APOSTAR
        usou_chave = False
    if random.random() < chance_vitoria:
        ganho = valor
        bonus_msg = ""
        if usou_chave:
            bonus_msg += " 🔑 **(Chave do Cofre garantiu a vitória!)**"
...
async def roubar(ctx, membro: discord.Member):
    ...
    if random.random() > 0.5:
        ...
        if u.get("imune_roubo"):
            u["imune_roubo"] = False
            registrar_evento(dados, "roubar_falhou", f"🚔 {ctx.author.display_name} tentou roubar {membro.display_name}, foi pego, mas usou a Carta de Imunidade.")
            await ctx.send(f"🛂 {ctx.author.mention} foi pego, mas a **Carta de Imunidade** evitou a multa! (**{restantes}/3** restantes hoje)")
        else:
...
async def usar(ctx, item: str):
    ...
    elif item == "dado_viciado":
        if u.get("dado_viciado_ativo"):
            await ctx.send("❌ Você já está com o Dado Viciado ativo!")
            return
        u["dado_viciado_ativo"] = True
        msg = "🎲 **Dado Viciado** ativado! Sua próxima aposta terá 50% de chance de vitória."
    elif item == "chave_cofre":
        if u.get("aposta_garantida"):
            await ctx.send("❌ Você já está com a Chave do Cofre ativa!")
            return
        u["aposta_garantida"] = True
        msg = "🔑 **Chave do Cofre** ativada! Sua próxima aposta vai ganhar garantidamente."
    elif item == "kit_pesca":
        if u.get("pesca_boost"):
            await ctx.send("❌ Você já está com o Kit de Pesca Turbo ativo!")
            return
        u["pesca_boost"] = True
        msg = "🎣 **Kit de Pesca Turbo** ativado! Sua próxima pescaria terá o ganho dobrado."
    elif item == "luva_caca":
        if u.get("caca_boost"):
            await ctx.send("❌ Você já está com a Luva do Caçador ativa!")
            return
        u["caca_boost"] = True
        msg = "🧤 **Luva do Caçador** ativada! Sua próxima caçada terá o ganho dobrado."
    elif item == "broca_diamante":
        if u.get("minerar_boost"):
            await ctx.send("❌ Você já está com a Broca de Diamante ativa!")
            return
        u["minerar_boost"] = True
        msg = "⛏️ **Broca de Diamante** ativada! Sua próxima mineração terá o ganho dobrado."
    elif item == "pingente_liberdade":
        if not u.get("procurado"):
            await ctx.send("❌ Você não está procurado, não precisa usar esse item agora!")
            return
        u["procurado"] = False
        u["recompensa_procurado"] = 0
        msg = "🕊️ **Pingente da Liberdade** usado! Seu status de Procurado foi removido."
    elif item == "carta_imunidade":
        if u.get("imune_roubo"):
            await ctx.send("❌ Você já está com a Carta de Imunidade ativa!")
            return
        u["imune_roubo"] = True
        msg = "🛂 **Carta de Imunidade** ativada! Você não pagará multa na próxima tentativa de roubo."
    u["inventario"][item] -= 1
    salvar(dados)
...
async def quiz(ctx, categoria: str = None):
    ...
    ops = "\n".join(pergunta["ops"])
    msg_texto = f"""🧠 **QUIZ INDIVIDUAL — {ctx.author.display_name}** {emoji} **{dif.upper()}** {f"| {nome_categoria}" if nome_categoria else ""}

**{pergunta['p']}**

{ops}

⏰ **20 segundos!** Só {ctx.author.mention} pode responder esse quiz.
✅ Acerto: **+{ganho} <:fichas:1517923672575185048>** | ❌ Erro: **-{perda} <:fichas:1517923672575185048>**"""
    view = QuizView(pergunta, ganho, perda, ctx.channel.id, ctx.author.id)
...
def _categorias_ajuda():
    categorias_str = ", ".join(f"`{k}`" for k in CATEGORIA_NOMES)
    return [
        {
            "chave": "minigames",
            "label": "Minigames & Diversão",
            "emoji": "🎮",
            "cor": discord.Color.green(),
            "campos": [
                ("🎣 `!pescar` `!caçar` `!minerar`", "Ganhe <:fichas:1517923672575185048> com mensagens engraçadas (3x por dia cada)", False),
                ("🎰 `!apostar [valor]`", "Aposta simples (mín. 50 <:fichas:1517923672575185048>)", False),
                ("🦹 `!roubar @pessoa`", "Tenta roubar alguém (3x por dia)", False),
                ("📅 `!diario`", "Recompensa diária com presente surpresa", False),
                ("✊ `!duelo @pessoa [valor]`", "Pedra, papel e tesoura apostado!", False),
                ("⚔️ `!lutar @pessoa`", "Desafia para uma luta com botões interativos", False),
            ],
        },
        {
            "chave": "quiz",
            ...
            "campos": [
                ("🧠 `!quiz [categoria]`", f"Quiz individual — só quem chamar responde!\nCategorias: {categorias_str}", False),
                ("🐉 `!chefe` / `!atacarchefe`", "Veja o chefe ativo do servidor e ataque-o (cooldown de 30s)", False),
                ("🎭 `!identidade`", "Recebe um papel secreto do dia por DM", False),
                ("🚨 `!procurado` / `!capturar @pessoa`", "Veja quem está procurado e tente capturar para a recompensa", False),
            ],
        },
        {
            "chave": "cassino",
            ...
            "campos": [
                ("🎰 `!casino`", "Lista todos os jogos de cassino disponíveis", False),
                ("🃏 `!blackjack` 🎡 `!roleta` 🎯 `!slots`", "Clássicos do cassino", False),
                ("🎲 `!dados` 🆙 `!hilo` ⚔️ `!guerra` 💥 `!crash`", "Mais jogos pra arriscar suas <:fichas:1517923672575185048>", False),
                ("ℹ️ Aposta mínima", "**50 <:fichas:1517923672575185048>** em todos os jogos", False),
            ],
        },
        {
            "chave": "banco",
            ...
            "campos": [
                ("🏦 `!investir [valor]`", "Investe suas fichas por 2 a 8 horas", False),
                ("💵 `!resgatar`", "Resgata o valor investido com rendimento", False),
                ("🎟️ `!bilhete [qtd]`", f"Compra bilhete(s) da loteria semanal por **{PRECO_BILHETE} <:fichas:1517923672575185048>** cada", False),
                ("🎰 `!loteria`", "Mostra o pote atual da loteria (sorteio automático todo domingo)", False),
            ],
        },
        {
            "chave": "economia",
            "label": "Economia & Perfil",
            "emoji": "💰",
            "cor": discord.Color.dark_gold(),
            "campos": [
                ("💰 `!saldo`", "Mostra suas <:fichas:1517923672575185048> e sua patente", False),
                ("👤 `!perfil [@pessoa]`", "Perfil completo com estatísticas", False),
                ("🎒 `!inventario`", "Itens guardados", False),
                ("🏆 `!top` / `!topsemana`", "Ranking geral dos mais ricos e ranking semanal", False),
                ("🤝 `!pagar @pessoa [valor]`", "Transfere <:fichas:1517923672575185048> para alguém", False),
            ],
        },
        {
            "chave": "loja",
            ...
            "campos": [
                ("🛒 `!loja`", "Loja de cargos exclusivos", False),
                ("🎁 `!itens`", "Loja de itens consumíveis com efeitos especiais", False),
                ("💳 `!comprar [item]`", "Compra um cargo ou item", False),
                ("✨ `!usar [item]`", "Usa um item do seu inventário", False),
            ],
        },
        {
            "chave": "progresso",
            "label": "Progresso & Patentes",
            "emoji": "🎯",
            "cor": discord.Color.purple(),
            "campos": [
                ("🎯 `!missoes`", "Veja suas missões diárias", False),
                ("🏅 `!conquistas [@pessoa]`", "Conquistas desbloqueadas", False),
        ...
        {
            "chave": "servidor",
            "label": "Servidor & Social",
            "emoji": "🌍",
            "cor": discord.Color.teal(),
            "campos": [
                ("☀️ `!clima` / 😄 `!humor`", "Veja o clima e o humor do dia (afetam seus ganhos)", False),
                ("📰 `!jornal`", "Últimas notícias e eventos do servidor", False),
                ("🌟 `!memorias`", "Marcos e momentos históricos do servidor", False),
                ("🤝 `!parcerias`", "Veja os servidores parceiros", False),
            ],
        },
    ]

def _arquivo_banner_ajuda():
    """Retorna um novo discord.File do banner do !ajuda, ou None se o arquivo não existir."""
    if os.path.isfile(BANNER_AJUDA_PATH):
        return discord.File(BANNER_AJUDA_PATH, filename=BANNER_AJUDA_NOME)
    return None

class AjudaSelect(Select):
    def __init__(self, autor_id, categorias):
        self.autor_id = autor_id
        self.categorias = {c["chave"]: c for c in categorias}
        options = [
            discord.SelectOption(label=c["label"], value=c["chave"], emoji=c["emoji"])
            for c in categorias
        ]
        super().__init__(placeholder="📂 Escolha uma categoria de comandos...", options=options)

    async def callback(self, interaction):
        if interaction.user.id != self.autor_id:
            await interaction.response.send_message("❌ Esse painel não é seu! Use `!ajuda` para abrir o seu.", ephemeral=True)
            return
        cat = self.categorias[self.values[0]]
        embed = montar_embed_ajuda(cat, interaction.guild)
        arquivo = _arquivo_banner_ajuda()
        if arquivo:
            await interaction.response.edit_message(embed=embed, view=self.view, attachments=[arquivo])
        else:
...
class AjudaView(View):
    def __init__(self, autor_id, categorias):
        super().__init__(timeout=120)
        self.message = None
        self.add_item(AjudaSelect(autor_id, categorias))
...
def montar_embed_ajuda(cat, guild):
    embed = discord.Embed(
        title=f"{cat['emoji']} {cat['label']}",
        color=cat["cor"]
    )
    for nome, valor, inline in cat["campos"]:
        embed.add_field(name=nome, value=valor, inline=inline)
    if guild and guild.icon:
        embed.set_thumbnail(url=guild.icon.url)
    if os.path.isfile(BANNER_AJUDA_PATH):
        embed.set_image(url=f"attachment://{BANNER_AJUDA_NOME}")
    embed.set_footer(text="🐅 Tigrinho Bot • Use o menu abaixo para ver outras categorias")
    return embed

@bot.command(name="ajuda")
async def ajuda(ctx):
    categorias = _categorias_ajuda()
    embed_inicial = discord.Embed(
        title="🐅 Painel de Ajuda do Tigrinho",
        description=(
            "Bem-vindo(a) ao painel de comandos! 🎉\n\n"
            "Use o **menu abaixo** para navegar pelas categorias e descobrir tudo que dá pra fazer por aqui: "
            "minigames, cassino, economia, loja, progresso e muito mais."
        ),
    ...
    for cat in categorias:
        embed_inicial.add_field(
            ...
            value=f"{len(cat['campos'])} comando(s)",
            inline=True
        )
    if ctx.guild and ctx.guild.icon:
        embed_inicial.set_thumbnail(url=ctx.guild.icon.url)
    if os.path.isfile(BANNER_AJUDA_PATH):
        embed_inicial.set_image(url=f"attachment://{BANNER_AJUDA_NOME}")
    embed_inicial.set_footer(text=f"Solicitado por {ctx.author.display_name} • Painel expira em 2 minutos", icon_url=ctx.author.display_avatar.url)

    view = AjudaView(ctx.author.id, categorias)
    arquivo = _arquivo_banner_ajuda()
    if arquivo:
        msg = await ctx.send(embed=embed_inicial, view=view, file=arquivo)
    else:
        msg = await ctx.send(embed=embed_inicial, view=view)
    view.message = msg
...
💰 ECONOMIA
  ...
  !cheattudo @p       → Cheat total (fichas + itens + conquistas)
...
🤝 PARCERIAS
  !painelparcerias    → Ver painel completo de parcerias
...
@bot.command(name="cheattudo")
async def cheattudo(ctx, membro: discord.Member = None):
    """Cheat completo: máximo de fichas, todos os itens, todas as conquistas e status liberados."""
    if ctx.author.id != DONO_ID:
        await ctx.send("❌ Você não tem permissão!")
        return
    membro = membro or ctx.author
    ...
    salvar(dados)
    await ctx.send(
        f"👑 **CHEAT TOTAL ATIVADO** para {membro.mention}!\n"
        f"💰 999999 <:fichas:1517923672575185048>\n"
        f"🎁 99x de cada item da loja\n"
        f"🏆 Todas as conquistas desbloqueadas\n"
        f"🧹 Cooldowns, status de Procurado e limites zerados"
    )
    await verificar_patente(ctx, u, dados)
...
@bot.command(name="painelparcerias")
async def painelparcerias(ctx):
    """Painel dedicado com todos os comandos de parcerias."""
    if ctx.author.id != DONO_ID:
        await ctx.send("❌ Você não tem permissão!")
        return
    dados = carregar()
...
📋 GERENCIAR
  ...
  !removerparceria [nome] → Remover parceria

📊 CONSULTAR
  !parcerias              → Ver tabela completa de parcerias

📈 STATUS ATUAL
  Total de parcerias firmadas: {total}
```""")
...
if not TOKEN or TOKEN == "TOKEN_DO_DISCORD":
    print("❌ ERRO: Token do Discord não configurado!")
    print("Crie um arquivo .env (copie de .env.example) e coloque seu token em DISCORD_TOKEN=...")
    raise SystemExit(1)
