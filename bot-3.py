...
def evento_aleatorio():
    ...
    if random.random() > 0.12:
        return None
    eventos = [
        ("🍀 Você encontrou {v} fichas no chão.", random.randint(20, 80)),
        ("🥾 Você tropeçou e perdeu {v} fichas.", -random.randint(5, 30)),
        ("📦 Um desconhecido lhe deu uma caixa com {v} fichas dentro.", random.randint(15, 60)),
        ("🍪 Você encontrou {v} fichas esquecidas em um bolso.", random.randint(20, 73)),
        ("🍪 Um desconhecido dividiu um biscoito com você. +{v} fichas.", 20),
        ("🌧️ Você ficou olhando a chuva por alguns minutos. +{v} fichas pela paz.", 5),
    ]
...
class QuizView(View):
    ...
    async def responder(self, interaction, escolha):
        ...
        if escolha == correta:
            ...
            registrar_evento(dados, "quiz", f"🧠 {interaction.user.display_name} acertou o quiz primeiro e ganhou {self.fichas_ganho} fichas.")
            salvar(dados)
            await interaction.response.edit_message(
                content=f"🏆 **{interaction.user.display_name}** acertou primeiro! A resposta era **{correta}**!\n💰 Ganhou **{self.fichas_ganho} fichas!**",
                view=None
            )
            ...
            for c in novas_conquistas:
                await interaction.followup.send(f"🏆 **Conquista desbloqueada por {interaction.user.display_name}:** {c['nome']} (+{c['recompensa']} fichas)")
        else:
            mensagens_extra = registrar_missao(u, "quiz")
            u["fichas"] = max(0, u["fichas"] - self.fichas_perda)
            salvar(dados)
            await interaction.response.send_message(
                f"❌ {interaction.user.mention} errou! Perdeu **{self.fichas_perda} fichas**. Outros ainda podem tentar!",
                ephemeral=False
            )
            for m in mensagens_extra:
                await interaction.followup.send(m)
...
async def sorteio_automatico():
    ...
    registrar_marco(dados, f"🎰 {nome} ganhou a loteria semanal automática e levou {premio} fichas!")
    dados["_loteria"] = {"bilhetes": {}, "pote": 0}
    ...
    if canal:
        ...
        await canal.send(f"🎉🎰 **SORTEIO SEMANAL!**\n{mencao} ganhou a loteria e levou **{premio} fichas**! Uma nova rodada começa agora.")
...
PEIXES = [
    ...
    ("tilápia enorme", "Não é muito bonita, mas vale fichas.", 0),
...
async def pescar(ctx):
    ...
    registrar_evento(dados, "pescar", f"🎣 {ctx.author.display_name} pescou um {peixe} e ganhou {ganho} fichas.")
    if e_tubarao:
        registrar_marco(dados, f"🦈 {ctx.author.display_name} pescou um TUBARÃO pela primeira vez!")
    salvar(dados)
    await ctx.send(f"{msg_pesca}\n💰 **+{ganho} fichas**{mult_str} (**{restantes}/3** restantes hoje)")
    if ev_aleatorio:
        texto_ev, delta_ev = ev_aleatorio
        sinal = "+" if delta_ev > 0 else ""
        await ctx.send(f"✨ *Evento aleatório:* {texto_ev} (**{sinal}{delta_ev} fichas**)")
...
async def cacar(ctx):
    ...
    if ev_aleatorio:
        texto_ev, delta_ev = ev_aleatorio
        u["fichas"] = max(0, u["fichas"] + delta_ev)
    registrar_evento(dados, "cacar", f"🏹 {ctx.author.display_name} caçou um {presa} e ganhou {ganho} fichas.")
    salvar(dados)
...
async def minerar(ctx):
    ...
    if ev_aleatorio:
        texto_ev, delta_ev = ev_aleatorio
        u["fichas"] = max(0, u["fichas"] + delta_ev)
    registrar_evento(dados, "minerar", f"⛏️ {ctx.author.display_name} minerou {minerio} e ganhou {ganho} fichas.")
...
async def roubar(ctx, membro: discord.Member):
    ...
    if random.random() > 0.5:
        ...
        novas_conquistas = verificar_conquistas(u)
        registrar_evento(dados, "roubar", f"🦹 {ctx.author.display_name} roubou {ganho} fichas de {membro.display_name}.")
        ...
        if roubos_hoje >= 3 and not u.get("procurado"):
            ...
            registrar_marco(dados, f"🚨 {ctx.author.display_name} ficou PROCURADO com recompensa de {recompensa_p} fichas!")
            await ctx.send(f"🚨 {ctx.author.mention} roubou demais e agora está **PROCURADO**! Recompensa: **{recompensa_p} fichas** para quem o capturar com `!capturar @{ctx.author.display_name}`!")
...
PRESENTES_DIARIOS = [
    {"nome": "🍫 Chocolate", "tipo": "fichas", "valor": 50, "desc": "Ganhou +50 fichas extras de bônus!"},
    {"nome": "🍫 Caixa de Chocolates Especial", "tipo": "fichas", "valor": 120, "desc": "Uma caixa inteira! +120 fichas de bônus!"},
    ...
    {"nome": "🪙 Nada especial", "tipo": "fichas", "valor": 0, "desc": "Só as fichas normais mesmo."},
]
...
async def diario(ctx):
    ...
    if presente["tipo"] == "fichas" and presente["valor"] > 0:
        ...
        msg_presente = f"\n🎁 **Presente:** {presente['nome']} — _Continha **{caixa_valor} fichas** dentro!_"
    ...
    novas_conquistas = verificar_conquistas(u)
    registrar_evento(dados, "diario", f"📅 {ctx.author.display_name} coletou o diário e ganhou {recompensa_base} fichas (streak {streak}).")
    salvar(dados)
    await ctx.send(f"📅 {ctx.author.mention} coletou a recompensa diária!\n💰 **+{recompensa_base} fichas** (sequência: **{streak} dia(s)**){msg_presente}")
    await verificar_patente(ctx, u, dados)
    for c in novas_conquistas:
        await ctx.send(f"🏆 **Conquista desbloqueada:** {c['nome']} (+{c['recompensa']} fichas)")
...
✅ Acerto: **+{ganho} fichas** (só pra quem acertar primeiro) | ❌ Erro: **-{perda} fichas** (pra cada um que errar)"""
...
async def investir(ctx, valor: int):
    if valor < 100:
        await ctx.send("❌ Valor mínimo para investir é **100 fichas**!")
        return
    dados = carregar()
    u = get_usuario(dados, ctx.author.id)
    ...
    if resultado == "lucro":
        ...
        descricao = f"📈 Lucro de {pct}% — retorno esperado: **{retorno} fichas**"
    elif resultado == "prejuizo":
        pct = random.randint(5, 25)
        retorno = int(valor * (1 - pct / 100))
        descricao = f"📉 Mercado instável... retorno esperado: **{retorno} fichas** (prejuízo de {pct}%)"
    else:
        retorno = valor
        descricao = f"➡️ Mercado estável. Retorno esperado: **{retorno} fichas** (sem lucro nem prejuízo)"
    u["investimento"] = {
        "valor": valor,
        "retorno": retorno,
    ...
    registrar_evento(dados, "investir", f"📊 {ctx.author.display_name} investiu {valor} fichas por {horas}h.")
    salvar(dados)
    await ctx.send(f"🏦 {ctx.author.mention} investiu **{valor} fichas** por **{horas} horas**!\n{descricao}\nUse `!resgatar` depois de {horas}h para receber o retorno.")
...
async def resgatar(ctx):
    ...
    registrar_evento(dados, "resgatar", f"🏦 {ctx.author.display_name} resgatou investimento: {sinal}{lucro} fichas ({retorno} no total).")
    salvar(dados)
    await ctx.send(f"🏦 {ctx.author.mention} resgatou o investimento!\n💰 Recebeu **{retorno} fichas** ({sinal}{lucro} de resultado)")
    await verificar_patente(ctx, u, dados)
...
async def capturar(ctx, alvo: discord.Member):
    ...
    registrar_evento(dados, "captura", f"🚔 {ctx.author.display_name} capturou {alvo.display_name} (procurado) e ganhou {recompensa} fichas.")
    registrar_marco(dados, f"🚔 {ctx.author.display_name} capturou o criminoso {alvo.display_name} e recebeu {recompensa} fichas!")
    salvar(dados)
    await ctx.send(f"🚔 {ctx.author.mention} capturou **{alvo.display_name}** e recebeu **{recompensa} fichas** de recompensa!")
...
`!pescar` — Pesca (itens e mensagens aleatórias!)
`!caçar` — Caça (presas engraçadas!)
`!minerar` — Minera (minérios variados!)
`!apostar [valor]` — Aposta suas fichas (mín. 50)
`!roubar @pessoa` — Tenta roubar alguém (3x/dia)
`!quiz [categoria]` — Disputa: quem acertar primeiro ganha!
`!diario` — Recompensa diária com presente surpresa
`!duelo @pessoa [valor]` — Pedra, papel e tesoura apostado!

🐉 **Chefe do Servidor**
`!chefe` — Ver o chefe ativo e top atacantes
`!atacarchefe` — Atacar o chefe (30s de cooldown)

🎭 **Identidade Secreta**
`!identidade` — Receba seu papel secreto do dia por DM

🏦 **Banco & Investimentos**
`!investir [valor]` — Investe por 2-8h (lucro ou prejuízo)
`!resgatar` — Resgata o investimento quando vencer

🎰 **Loteria Semanal** (sorteio automático todo domingo)
`!bilhete [qtd]` — Comprar bilhetes (100 fichas cada)
`!loteria` — Ver o pote atual

🌍 **Servidor**
`!clima` — Clima do dia e seus efeitos
`!humor` — Humor do bot hoje
`!jornal` — Últimos acontecimentos
`!memorias` — Momentos épicos registrados
`!parcerias` — Placar de parcerias do servidor

🚨 **Procurado**
`!procurado` — Ver quem está procurado
`!capturar @pessoa` — Capturar e ganhar recompensa
...
💰 **Economia**
`!saldo` — Fichas e patente
`!perfil [@pessoa]` — Perfil completo
`!inventario` — Itens guardados
`!top` — Ranking dos mais ricos
`!pagar @pessoa [valor]` — Transferir fichas
`!loja` — Loja de cargos
`!itens` — Itens consumíveis
`!comprar [item]` / `!usar [item]` — Comprar e usar itens

🏅 **Patentes:** 🧢 10k | 💵 50k | 💎 100k | 🏦 500k

⚔️ **Lutas:** `!lutar @pessoa` — use os botões para lutar"""
...
async def sortearloteria(ctx):
    ...
    nome = membro.display_name if membro else f"<@{vencedor_id}>"
    registrar_marco(dados, f"🎰 {nome} ganhou a loteria semanal e levou {premio} fichas!")
    dados["_loteria"] = {"bilhetes": {}, "pote": 0}
    salvar(dados)
    bonus_str = f" _(clima noturno: x{mult_lot:.2f})_" if mult_lot != 1.0 else ""
    await ctx.send(f"🎉🎰 **{nome}** ganhou a loteria semanal e levou **{premio} fichas**{bonus_str}! Uma nova rodada começou.")
...
EVENTOS_SERVIDOR = [
    ...
    {"nome": "🔥 Grande Incêndio",      "desc": "As fichas queimam! Todos perdem um pouco.",   "tipo": "todos_perdem",  "valor": 70},
...
class DueloView(View):
    ...
    async def aceitar(self, interaction, button):
        ...
        if u_desaf["fichas"] < self.valor or u_desal["fichas"] < self.valor:
            await interaction.response.edit_message(content="❌ Um dos jogadores não tem fichas suficientes!", view=None)
            return
        ...
        if escolha_a == escolha_b:
            resultado = "Empate! As fichas ficam onde estão."
            ...
            resultado = f"🏆 **{desafiante.display_name if desafiante else 'Desafiante'}** venceu e ganhou **{self.valor} fichas!**"
        else:
            ...
            resultado = f"🏆 **{interaction.user.display_name}** venceu e ganhou **{self.valor} fichas!**"
...
async def duelo(ctx, membro: discord.Member, valor: int):
    ...
    if membro.bot:
        ...
        return
    if valor < 50:
        await ctx.send("❌ Valor mínimo do duelo é **50 fichas**!")
        return
    dados = carregar()
    u = get_usuario(dados, ctx.author.id)
    if u["fichas"] < valor:
...
async def invocar(ctx):
    ...
    await ctx.send(
        ...
        f"💰 Prêmio total: **{premio_total} fichas** (dividido entre quem mais atacou)\n"
...
async def atacarchefe(ctx):
    ...
    if golpe_final:
        ...
        msgs = [f"💀 **{chefe['nome']} foi derrotado!**\n\n🏆 {ctx.author.mention} desferiu o **golpe final** e ganhou **{bonus_final} fichas** de bônus!\n\n📊 **Distribuição do prêmio:**"]
        for uid_d, dmg in sorted(chefe["danos"].items(), key=lambda x: x[1], reverse=True):
            ...
            msgs.append(f"• **{nome_p}** — {dmg} de dano ({pct}%) → **+{parte} fichas**")
...
async def evento_automatico():
    ...
    if evento["tipo"] == "todos_ganham":
        ...
        registrar_marco(dados, f"🌟 Evento: {evento['nome']} — todos ganharam {evento['valor']} fichas!")
        salvar(dados)
        await canal.send(
            f"⚡ **EVENTO DO SERVIDOR!**\n\n"
            f"**{evento['nome']}**\n"
            f"_{evento['desc']}_\n\n"
            f"🎉 Todos os jogadores ganharam **+{evento['valor']} fichas!**"
        )
    elif evento["tipo"] == "todos_perdem":
        for uid in membros_ativos:
            u = get_usuario(dados, uid)
            u["fichas"] = max(0, u["fichas"] - evento["valor"])
        registrar_marco(dados, f"⚠️ Evento: {evento['nome']} — todos perderam {evento['valor']} fichas!")
        salvar(dados)
        await canal.send(
            f"⚡ **EVENTO DO SERVIDOR!**\n\n"
            f"**{evento['nome']}**\n"
            f"_{evento['desc']}_\n\n"
            f"💸 Todos os jogadores perderam **{evento['valor']} fichas!**"
        )
        ...
        registrar_marco(dados, f"⚡ Tempestade redistributiva — {redistribuicao} fichas de {nome_rico} foram distribuídas!")
        salvar(dados)
        await canal.send(
            f"⚡ **EVENTO DO SERVIDOR!**\n\n"
            f"**{evento['nome']}**\n"
            f"_{evento['desc']}_\n\n"
            f"🌀 **{redistribuicao} fichas** foram redistribuídas de **{nome_rico}** para todos!"
        )
